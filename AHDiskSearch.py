...
def get_sub_string(data: list, query_append, prefix='filename = ', like_query=False):
    try:
        ...
        return prefix + f'{query_append}'.join(f"'%{word}%'" if like_query else f"'{word}'" for word in result.split(f'{query_append}'))
    except:
        return ""
...
searchButton = ttk.Button(search, text="Search", width=1, command=show, style='primary.TButton').grid(row=2, column=2,
                                                                                                      sticky=tk.EW,
                                                                                                      padx=(10, 1))
configButton = ttk.Button(search, text="Config", width=1, command=iconfig, style='primary.TButton').grid(row=2,
                                                                                                         column=3,
                                                                                                         sticky=tk.EW,
                                                                                                         padx=(1, 1))
exitButton = ttk.Button(search, text="Exit", width=1, command=exit, style='secondary.TButton').grid(row=2, column=4,
                                                                                                    sticky=tk.EW,
                                                                                                    padx=(1, 1))
